...
class Solver {
		   ...
		   double[] alpha_, double Cp, double Cn, double eps, SolutionInfo si, int shrinking)
	{
		...
		int iter = 0;
		int max_iter = Math.max(100000, l>Integer.MAX_VALUE/100 ? Integer.MAX_VALUE : 100*l);
		max_iter = Math.min(max_iter, 20000);
		int counter = Math.min(l,1000)+1;
		int[] working_set = new int[2];

//		System.out.println("haha" + max_iter);
		while(iter < max_iter)
		{
