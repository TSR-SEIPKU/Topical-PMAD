 ...
 * @since 3.2
 */
public class MyMixMND
    extends MixtureMultivariateRealDistribution<MyMND> {
     ...
     */
    public MyMixMND(double[] weights,
                                                 double[][] means,
                                                 double[][][] covariances) {
     ...
     */
    public MyMixMND(List<Pair<Double, MyMND>> components) {
        super(components);
    }
     ...
     * number of variables.
     */
    public MyMixMND(RandomGenerator rng,
                                                 List<Pair<Double, MyMND>> components)
     ...
     */
    private static List<Pair<Double, MyMND>> createComponents(double[] weights,
                                                                                       double[][] means,
                                                                                       double[][][] covariances) {
        final List<Pair<Double, MyMND>> mvns
            = new ArrayList<Pair<Double, MyMND>>(weights.length);

        for (int i = 0; i < weights.length; i++) {
            final MyMND dist
                = new MyMND(means[i], covariances[i]);

            mvns.add(new Pair<Double, MyMND>(weights[i], dist));
        }
